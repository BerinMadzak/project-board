...
export default function TaskCard({ task, projectMembers }: Props) {
  ...
  const onSubmit = async (data: TaskForm) => {
    setIsSaving(true);
    await dispatch(
      updateTask({ id: task.id, ...data, dueDate: data.dueDate ? new Date(data.dueDate) : null, projectId: task.projectId }),
    );
    setIsEditing(false);
    setIsSaving(false);
  };
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} className="group">
      <div className="rounded-lg bg-gray-800 border border-white/5 hover:border-white/10 transition-colors p-3">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); dispatch(deleteTask({ id: task.id })); }}
        ...
        <p className="text-sm text-white font-medium leading-snug mb-1">{task.title}</p>
      ...
      {isEditing &&
        createPortal(
          <div
            onClick={(e) => { if (e.target === e.currentTarget && !isSaving) setIsEditing(false); }}
