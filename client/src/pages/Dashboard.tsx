        ...
        {projects.length > 0 && selectedProject && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2.5 rounded-lg bg-white/5 border border-white/10 hover:border-white/20 px-4 py-2.5 text-sm font-medium text-white transition-colors cursor-pointer"
            >
              <span
                ...
                <div className="absolute right-0 top-full mt-1.5 z-20 w-52 rounded-xl bg-gray-900 border border-white/10 shadow-2xl overflow-hidden">
                  {projects.map((project) => (
                    <button
                      ...
                      className={`flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-left transition-colors cursor-pointer hover:bg-white/5 ${
                    ...
                    >
                      ...
                      {project.id === selectedProjectId && (
                        <svg
                          className="ml-auto h-3.5 w-3.5 text-indigo-400"
                          viewBox="0 0 20 20"
                          fill="currentColor"
                        >
                          <path
        ...
        <div className="animate-pulse space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="rounded-xl bg-white/5 border border-white/10 h-24"
              />
            ))}
          </div>
            ...
            <div className="rounded-xl bg-white/5 border border-white/10 p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="text-xs text-gray-500">
                  {data.stats.completed}/{data.stats.total} tasks
                </span>
              </div>
                  ...
                  <div className="h-1 rounded-full bg-white/5 overflow-hidden">
                    <div
                      ...
                      style={{
                        width:
                          data.stats.total > 0
                            ? `${Math.round((value / data.stats.total) * 100)}%`
                            : "0%",
                      }}
                    />
                  </div>
            ...
            {data.tasks.length === 0 ? (
              ...
              data.tasks.map(({ title, status }, i) => (
                ...
                >
                  ...
                  <span
                    className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${statusConfig[status]?.className ?? statusConfig.TODO.className}`}
                  >
      ...
      {!loading && !error && projects.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <button
            ...
            className="rounded-lg bg-indigo-500/20 border border-indigo-500/30 px-4 py-2 text-sm font-medium text-indigo-300 hover:bg-indigo-500/30 transition-colors cursor-pointer"
          >
            Go to Projects
          </button>
        </div>
